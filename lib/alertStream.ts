...
export function openAlertStream(
  ...
  async function connect() {
    ...
    try {
      token = await api.getAuthToken(forceToken);
    } catch {
      if (ticket === sequence) retry();
      return;
    }
    ...
    request = xhr;
    xhr.open("GET", `${api.getApiBase()}/notifications/stream?role=client`);
    ...
    xhr.onreadystatechange = () => {
      ...
      if (xhr.status !== 200) {
        ...
        if (xhr.status === 409) {
          cursor = null;
          handlers.onStatus?.(false);
          void Promise.resolve(handlers.onResumeUnavailable?.())
            .catch(() => {})
            .finally(() => {
              if (!closed && ticket === sequence) retry();
            });
        } else {
          forceToken = xhr.status === 401;
          retry();
        } // Includes 401: next connect obtains a fresh Clerk JWT.
        return;
      }
      forceToken = false;
      if (!announced) {
