...
- `lib/navigationHeaders.ts` — multi-origin stack pushes label the back control **"Back"** (`headerBackTitle`), the one word true from every origin, so iOS never writes `(tabs)` or a single origin's title there. A bare chevron (`minimal`) was also honest and was reported as "no Back".
