...
- **Money** — PHP minor units only. **Breakdown policy (captain, 2026-09-22): a shelf price — catalog, match, listing, a basket line or run header — is GRIDGO's price (shop figure + live `serviceFeeRateBps`, drawn through `GridgoPrice`), while the breakdown on checkout, order detail and the receipt names the shop's Printing figure, Delivery and a tappable "Service fee · N%" row (`components/ServiceFeeRow.tsx`, `lib/serviceFee.ts`, explainer on tap) that add up to the same Total.** The fee rate comes from `GET /settings` (or the order's snapshotted `serviceFeeRateBps`) — never a hardcoded 3%. The shop's own settlement is still withheld. The acknowledgement receipt is `GET /orders/:id/invoice`, opened from checkout success and order detail (`app/order/receipt.tsx`).
