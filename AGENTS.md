...
- **Checkout is QR Ph, a reference and a receipt.** `POST /me/carts/:id/checkout` takes `{method:"qr_manual", reference, proofFileId}` and nothing else; the screenshot is a `payment_proof` upload (`hooks/usePaymentProof.ts` — images only, 15 MB, a different limit from artwork). Receipt bytes, OCR, and the typed reference live in `store/checkoutPayment.ts` keyed by cart id — leaving checkout to set an address must not drop them. It writes the order at `needs_qa` with the 75/25 split already submitted, so the order screen shows "In artwork check" and asks for nothing. Note those payments are keyed `initial` / `final_online`, **not** the `downpayment` / `balance` that `lib/payment.ts` and `components/PaymentPanel.tsx` still expect — that gap only bites when one of these orders becomes payable.
