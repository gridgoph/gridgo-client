 ...
 * identity onto it.
 *
 * A sideloaded APK has no store listing, so the version a client reads in
 ...
 *
 * This mirrors gridgo-rider's app.config.ts — keep the three apps identical.
 */
 ...
 *
 * Push on Android needs the Firebase config for `ph.gridgo.client` in project
 * `gridgo-c2ce9`. That file is the captain's and is **never committed** —
 * `.gitignore` keeps it out — so it reaches a build as configuration:
 *
 ...
 * is present before it calls prebuild.
 *
