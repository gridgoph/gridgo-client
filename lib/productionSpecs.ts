...
export function productionSpecRows(item: ProductionItem, describeValue: (value: string) => string = readableSpec): SpecValue[] {
  ...
  for (const [key, value] of Object.entries(item.structuredSpec ?? {})) {
    ...
    const text = typeof value === "string" ? describeValue(value) : typeof value === "number" ? String(value) : typeof value === "boolean" ? (value ? "Yes" : "No") : null;
    if (!text) continue;
