 ...
 * two editors cannot drift. Permission is requested only on the tap — a quiet
 * pre-pin runs solely when the phone has already granted location and the
 * map is still empty.
 */
...
export function useDropoffEditor(options?: {
  ...
  const runReverse = useCallback(async (next: GeoPoint, fillStreet: boolean) => {
    const result = await reverseNominatim(next, appVersion());
    if (result.status === "ok") {
      if (fillStreet || !lineRef.current.trim()) setLine1(result.suggestion.line1);
    ...
    if (fillStreet && !lineRef.current.trim()) setNotice(STREET_UNREAD);
  }, []);
