...
- **Home is a summary, not a second orders list.** `app/(tabs)/home.tsx` answers two questions: is anything waiting on me (`HomeActionRow` docket, verb first), and how do I start something new (category tiles + the yellow "+"). The Orders tab owns the full `OrderCard` list, search, filters, sort and reorder. Do not put those cards back on Home.
