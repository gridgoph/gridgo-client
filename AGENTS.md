...
- React Native
- TypeScript
- Expo Router — navigation types come from `expo-router/react-navigation` and `expo-router/js-tabs`. Do not re-add `@react-navigation/native` or `@react-navigation/bottom-tabs`.
- NativeWind
- Zustand
- AsyncStorage
...
- **Money** — PHP minor units only. **Breakdown policy (captain, 2026-09-22 evening, supersedes the morning rule): GRIDGO’s cut in pesos appears on no client screen — every figure a client reads carries the fee inside it, so a shelf price (catalog, match, listing, a basket line or run header) is the shop figure + live `serviceFeeRateBps` drawn through `GridgoPrice`, and on checkout, order detail and the receipt Printing is the fee-inclusive amount (`printingMinor` in `lib/serviceFee.ts`, `clientItemSubtotalMinor` on a basket) with a tap-to-explain `Service fee · N%` row carrying no amount (`<ServiceFeeRow explainOnly>`), so Printing + Delivery = Total.** The fee rate comes from `GET /settings` (or the order's snapshotted `serviceFeeRateBps`) — never a hardcoded 3%. The shop's own settlement is still withheld. The acknowledgement receipt is `GET /orders/:id/invoice`, opened from checkout success and order detail (`app/order/receipt.tsx`).
- **Pilot switches live in `constants/features.ts`** — one boolean per held-back feature, code left in place. `PHYSICAL_INVOICE_REQUESTS_ENABLED` is off for the pilot (gridgoph/gridgo-web#61): no screen offers a printed-invoice request, but an order that already has one still links to it through `physicalInvoiceEntry` (`lib/physicalInvoice.ts`). Turn an entry point off through the switch, never by deleting it.
...
- `lib/readyTime.ts` + `components/ReadyTime.tsx` — ready dates use the API's client `promiseBy` (match, cart line, order), formatted in Davao time. Listing `turnaroundHours` is press time only (`printTimeLine` in `lib/listing.ts`); never substitute it, a requested deadline, or a supplier date for the client promise.
- **Home is a summary, not a second orders list.** `app/(tabs)/home.tsx` answers two questions: is anything waiting on me (`HomeActionRow` docket, verb first), and how do I start something new (one START A PRINT section: `HomeSearchEntry` into the picker, then the five categories as one full-width board of `HomeCategoryRow`s, with the yellow "+" still the primary start). The Orders tab owns the full `OrderCard` list, search, filters, sort and reorder. Do not put those cards back on Home. **The search bar belongs inside that section, never pinned above the docket** — a bar at the top of the screen answers question two first. Nothing in the column is yellow. The two jobs sections are deliberately different shapes, because they are different questions: **needs-you is a flush docket** of dense verb-first rows ("which of these, and what do I do"), each led by a tone mark — one icon on a soft disc of its own semantic colour — and followed by the job's name in body rather than muted caption, because two jobs can be waiting on the very same verb ("Collect at GRIDGO Office", twice) and then the name is all that tells them apart. **That mark is keyed to the action, never to the state it was asked from**: the balance is asked for from `ready_for_dispatch` onward, whose own chip is an info-blue clock, and a blue clock over "Pay the remaining 25%" tells a client to wait for the one thing waiting on them. `tone`/`icon` on `OrderNextAction` (`lib/orderState.ts`) own that; `HomeRow` owns the Lucide registry, the same lib/UI split as `StatusChip`. Colour still never carries it alone — the verb says the same thing in words. **The jobs slot is three groups, split by `lib/homeJobs.ts`, in this order: NEEDS YOU (every waiting job), IN PROGRESS (capped cards), RECENTLY FINISHED (capped one-line rows).** Three finished jobs drawn as full cards looked exactly like live work and pushed the start menu off the first screen (issue #91). `HomeJobRow` is the in-progress card ("where has it got to"): led by the precise state in words with its tone icon, the job name under it, `OrderStageRail`'s `compact` variant, then `orderWaitingOn` and the client's `promiseBy` through `ReadyTime` — never the supplier's `promisedDate`. The rail is coarse by design and the headline is the exact state. An unknown state is live, never finished, and draws no rail. `HomeFinishedRow` is a flush row: check mark, name, "Completed yesterday"; no rail, no chip. "View all" rides on the first of those two sections. Money, quantity and reorder stay off both; "View all" (the one place the `brand` token is spent, as a small link) goes to Orders. Every section opens with the same `SectionHead` — quiet overline, an optional neutral count pill where the number is the point (NEEDS YOU answers "is anything waiting on me" before a row is read), and the way out on the right. The label stays its own text node; a count concatenated into it would make "NEEDS YOU" a string that moves with the data. **A Home with nothing on press is the one place Home introduces the platform**, and its start section reads START YOUR FIRST PRINT. It greets the client by first name — the header row still never carries it — and draws `HomeSampleStrip`: real shop samples at real "From" prices, picked by `lib/homeSamples.ts` one per family in tree order, preferring a photographed listing over a cheaper blank one, so the strip says what GRIDGO makes rather than what one family makes. It is not the grey "No print jobs yet" slab that used to sit there — that spent Home's best space on a button the start board already is. It reads only `HOME_BOARDS` boards (`loadCategoryBoards("", { maxBoards })`, cache-keyed by the cap so a short Home read cannot shorten the category screen's), only when there are no orders at all, and is left out entirely when the read fails or comes back empty: an empty shelf is worse than no shelf. No shop is named on it, and there is still no how-it-works strip. **The start menu is a board, never a grid**: five families in two columns left the fifth alone beside an empty half-row, halved every target, and truncated the contents line mid-item on three of five ("Plaques & Trophies and 2 …") — that line is the one a person actually reads, so it must be set whole. One column of hairline-divided rows inside `gg-card-flush` is what fits a phone. `lib/categoryLook.ts` holds the two pure lookups the row needs — the short label (the governed names do not fit a menu at any width; the catalogue name stays the accessibility label and stays the heading on `app/request/[category].tsx`) and the glyph *name*, with `HomeCategoryRow` owning the Lucide registry, the same lib/UI split `lib/orderState.ts` keeps with `StatusChip`. The row's mark sits in `CropMarkFrame`, so the menu and the shops' real sample photos read as one board rather than a borrowed product list — and its sheet shape is what tells these rows apart from the docket above, whose marks are discs. Do not give the docket a crop-marked well, or the two lists become one.
- **`lib/gridgoPrice.ts` (`clientAmountMinor`) + `components/GridgoPrice.tsx` — every peso a client reads is GRIDGO's price, never the shop's.** The API sends the shop's own figures (`basePriceMinor`, `fromPriceMinor`, `effectivePriceMinor`, option modifiers, a cart line's `lineSubtotalMinor`) because the supplier and ops apps read the same endpoints; those are what the shop is paid. GRIDGO's charge (`settings.serviceFeeRateBps`, `store/platformSettings.ts`, read by `useGridgoCharges` at sign-in) is added on the client at draw time with the platform's own `roundBps` (`lib/gridgoPrice.ts` is the same arithmetic taking the API's `clientFromPriceMinor` first); no client surface names the shop’s figure or the fee in pesos — see **Money** above. Draw a shelf price through `GridgoPrice` (it waits on a skeleton until the rate is known) or `clientAmountMinor`; never `formatPhp` a supplier field directly. `GridgoPrice.supplierMinor` takes only an unmarked shop amount: passing a client amount adds the fee twice. The rendered listing/checkout parity regression is `app/__tests__/listing-price-consistency.test.tsx`. Two things the reported ₱0.00 basket taught: **`lineSubtotalMinor: null` is a real answer** (GRIDGO's pricer refused the line — usually a quantity under `minimumOrderQuantity`) and is drawn as the reason (`unpricedLineReason`) with Place order blocked, never as zero; and the order sheet opens at the shop's minimum and will not step under it, while gridgo-api refuses an under-minimum quantity at line add/save with `409 below_minimum_quantity`.
- `lib/payment.ts` — the payment plan: which payment is payable, which is with Operations, reference validation, and the copy that says GRIDGO takes QR only. **New orders are paid in full up front** (gridgo-api#66): the order snapshots `downpaymentPercent: 100`, `balanceMinor: 0`, and its balance installment is `not_required`. `paysInFull(order)` reads any of the three; legacy 75/25 orders keep their balance step. Every percentage and every "downpayment"/"balance" word a client reads goes through this module (`downpaymentPercentOf`, `installmentLabel`, `payActionTitle`, `paymentPlanNote`) — a basket reads `GET /settings` `downpaymentPercent` via `settingsDownpaymentPercent`, which falls back to 75 only because an API without the field still writes 75/25. Never write "75%" in a screen. Installment statuses are open strings: an unknown one must never crash. **Cash on delivery and Pilot Credits are not payment methods** — both routes are `410`/`400` server-side, and offering either walks a client into an error. Balance is asked for from `ready_for_dispatch` onward, not the moment the downpayment clears, or the split is a fiction
...
- `lib/accountProfile.ts` — the account after sign-up: the identity card's headline (a business is its business name; **never inferred from `orgName`**, same rule as the lockup), the details form, and the stepped business application. Contract is `src/account-profile-routes.js` and `POST /me/business-application` in **gridgo-api** — `GET /me` / `PATCH /me` (**`expectedVersion` is required**, and a 409 is only stale when the code is `account_version_conflict`; `/me` also answers 409 for an identity with no client profile) return `{ user, approvalCase }`. Applying does **not** flip `accountType`; Operations approves the case first. Account is the one home for the match ranking; Settings keeps theme and onboarding. Nothing writes an account type locally: the next `/me` would overwrite it, and meanwhile the client is told they are something the platform has not heard of
...
- **Checkout is QR Ph, a reference and a receipt.** `POST /me/carts/:id/checkout` takes `{method:"qr_manual", reference, proofFileId}` and nothing else; the screenshot is a `payment_proof` upload (`hooks/usePaymentProof.ts` — images only, 15 MB, a different limit from artwork). Payment draft ownership and receipt OCR are documented in [docs/RECEIPT_OCR_VALIDATION.md](docs/RECEIPT_OCR_VALIDATION.md#draft-ownership). It writes the order at `needs_qa` with the up-front payment already submitted, so the order screen shows "In artwork check" and asks for nothing. Note those payments are keyed `initial` / `final_online`, **not** the `downpayment` / `balance` that `lib/payment.ts` and `components/PaymentPanel.tsx` still expect — that gap only bites when one of these orders becomes payable.
...
- `lib/notificationPresentation.ts` + `components/NotificationCard.tsx` — one inbox card per job (`groupInbox`), read top to bottom as stage meter + label and relative time → one headline and two lines of body → at most one toned `callout` (the thing to do or know; the presentation layer owns its copy, tone and icon, and drops a type's ask once the live `orderState` has moved past it) → reference and job name. Unread is an ink spine, never yellow. Earlier updates are `timelineRows`: a day heading once, a time only where it changes. Draw thin connectors against `textMuted`, not `outline` — `outline` is within a shade of `surfaceHigh` in Dark.
...
- `lib/printerWidth.ts` — a tarpaulin listing's `printerMaxWidthFeet` ("Prints up to 7 ft wide") and the over-width warning on the listing sheet. `requestedWidthFeet` mirrors gridgo-api's reading (measurement → bound size → option label, first number of "4x8" is the width) so the warning agrees with `409 printer_cap_exceeded`. No cap or no width is never warned about.
...
- `lib/appUpdate.ts` — the in-app update prompt, shared verbatim with gridgo-rider and gridgo-supplier (only `APP_UPDATE_SOURCE` names this app). It reads GitHub's `releases/latest` unauthenticated, takes the `versionCode` from the tag's run number, and offers `https://gridgo.talasora.com/downloads/gridgo-client.apk`; "Update completed" is said on the next launch because Android's installer owns the install. While the phone is behind, the sheet returns on every cold launch and on a foreground once the check interval has passed (`shouldShowUpdatePrompt`; "Later" is in memory only), and the Notifications tab pins a local card for it (`components/AppUpdateNotices.tsx`) plus a one-time "Updated to version X" item — neither touches gridgo-api or the unread count. Only a CI-stamped build (`1.0.<n>` / `versionCode` n) is compared; Expo Go, dev and local builds are skipped unless `EXPO_PUBLIC_UPDATE_CHECK_FORCE_VERSION_CODE=<n>` pretends to be build n. Failure is silence to the client, never a message; a development build logs every decision as an `[update-check]` line in Metro (`logUpdateCheck`), because a prompt that silently never appears is otherwise undiagnosable. The read sends its own `User-Agent`: GitHub answers a request without one with 403, the same status as its rate limit.
- `lib/bugReport.ts` + `app/report-problem.tsx` — Account → Report a problem. A bug report is a support-chat message (`newThread: true`) headed `BUG_REPORT_HEADING`, with app version, phone, OS and an optional order reference shown on screen before sending; it lands the client in that thread. Chat is text-only, so there is no screenshot until gridgo-api grows a chat file purpose and the ops desk renders it. Never send the Android serial or build fingerprint.
...
- Every pushed root-stack screen uses `pushedScreenOptions("<title>")`. [lib/navigationHeaders.ts](lib/navigationHeaders.ts) owns the shared header contract; [lib/__tests__/pushedRouteLayout.test.ts](lib/__tests__/pushedRouteLayout.test.ts) renders the root layout and checks effective options for signed-in and signed-out routes.
- Pushed screen content must not add another top inset. See the edge-ownership contract in [components/FormScreen.tsx](components/FormScreen.tsx); only tab screens and full-bleed routes own their top edge.
...
- **The permission is asked once or twice and a refusal is effectively permanent.** So nothing in the app may call `requestPermissionsAsync` on launch; the only callers are taps on `components/PushExplainerSheet.tsx` and `components/PushEnableCard.tsx`. The explainer is drawn once a signed-in client lands on the tab shell (never over auth, profile, ranking, the intro or the update prompt) and at most every 7 days after (`pushExplainerDue` in `lib/push.ts`, stamp in `store/pushPrompt.ts`, written when it opens). The card is the way back, drawn on Home, the Notifications tab and an order waiting on somebody else; once `canAskAgain` is false both hand over to `Linking.openSettings()`, and `usePush.resume` on every foreground re-reads permission and registers. **Login must not import either** — Expo Go Android SDK 53 throws when `expo-notifications` is first imported, and the card pulls in `store/push`. Once blocked, `Linking.openSettings()` is the only honest offer — the app cannot raise the dialog again.
...
- **The device token goes with `POST /auth/logout`, not to `/devices/unregister`.** After sign-out the bearer token is dead, so a phone that signed out first could no longer authenticate an unregister and would keep waking for the previous person's orders. Logout then **releases** the row (it does not delete it). The client captures the outgoing bearer before local teardown and serializes logout with device registration; see `store/session.ts` and `lib/__tests__/logoutBearer.test.ts`.
...
- Push navigation and account boundaries are specified in [Live resource updates](docs/REALTIME_UPDATES.md); preserve the readiness and ownership guards in `hooks/usePushNotifications.ts`.
...
- Start commands, Expo Go compatibility and development-build setup live in [README.md](README.md#getting-started). Preserve the scripts in `package.json` after local prebuilds.
- `npx tsc --noEmit`, `npx jest`, `npx expo lint` all have to be clean.
- **Jest must exit.** A suite that reports then sits on an open handle (Reanimated `withRepeat`, a stray `fetch` to Nominatim) is how GitHub cancelled Config and tests at `timeout-minutes: 20`. `forceExit` is on in the Jest config and the check job; do not take it off. `jest.setup.js` stubs `global.fetch` — unit tests spy on it, they do not call the network. A screen that uses `useAuthLanding` must mock `@clerk/expo` (`app/__tests__/complete-profile.test.tsx`).
...
- jest-expo still mocks the `ImageLoader` native module in the **old callback shape** while React Native calls the promise one, so any screen that measures artwork with `Image.getSize` throws `success is not a function` from a `nextTick` — outside every promise chain the screen could catch. Stub `Image.getSize` in that screen's test (`app/__tests__/artwork.test.tsx`).
...
- **The APK clients sideload is built, proved, and published by CI.** `.github/workflows/android-release.yml` — the same workflow as gridgo-rider's, so keep the two in step: a merge to `main` (or a manual dispatch) runs `expo prebuild` → `assembleRelease` → `scripts/verify-release-apk.sh` → GitHub Release and `upload-apk client` from the on-disk APK, then the run artifact (`continue-on-error`: a full Actions artifact quota must not leave the landing site stale). The deploy key's forced command is `upload-apk client` (bytes on stdin, digest echoed back and compared). `android/` is generated, never committed — `.gitignore` keeps it out and a checked-in copy drifts from `app.json` on every Expo upgrade — so `npx expo config --type public` has to gate the build. Note `expo prebuild` also rewrites `package.json`'s `android`/`ios` scripts to `expo run:*`; throwaway in CI. Locally, preserve the scripts owned by `package.json`; see [README.md](README.md#getting-started). Two failures are invisible in a green log and both are asserted against the built APK rather than trusted: Expo's generated `android/app/build.gradle` points the release build type at `signingConfigs.debug`, so signing comes from AGP's injected config (a properties file in `~/.gradle`, never the gradle command line, which would put the password in the process list) and the APK's certificate is then compared to the keystore alias; and **`EXPO_PUBLIC_*` values are build-time configuration**, so `EXPO_PUBLIC_API_URL`, `EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY`, and `EXPO_PUBLIC_CARTO_API_KEY` must be on the release job's `expo config`, `expo prebuild`, Gradle, and verify steps. Config/prebuild stamps Clerk into `extra.clerkPublishableKey` when the env is present; `lib/clerkAuth.ts` `resolveClerkPublishableKey` also keeps a static `process.env.EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY` read so Babel can inline the live value at `assembleRelease` if extra was empty. Never print the key. Release rejects anything except `pk_live_*`, and the verifier requires the API URL, Clerk key, and CARTO key inside `assets/index.android.bundle` (`lib/cartoTiles.ts` must read the literal `process.env.EXPO_PUBLIC_CARTO_API_KEY` so Babel inlines it; a member access on `env` ships a keyless URL). The verifier greps the key quietly like Clerk — a `?key=` shape alone is not enough, because the bundle already contains other query strings. It must not print the CARTO key. A missing CARTO secret fails the Gradle preflight rather than shipping the "API KEY REQUIRED" watermark. `__tests__/releaseWorkflow.test.ts` and `__tests__/verifyReleaseApk.test.ts` pin the workflow and built-artifact behavior. Triggers earn different outputs: a pull request gets config, typecheck and tests and references no secret at all; only the default branch names a Release or replaces what the download page serves.
