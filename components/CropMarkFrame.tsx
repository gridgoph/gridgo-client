...
type Props = {
  ...
  /**
   * Clip the well to its box. Sample photos cover-crop inside it; a contain-fitted
   * payment plate must pass `false` or the JPEG's QR is sliced off.
   */
  clip?: boolean;
};
 ...
 * once already.
 */
export function CropMarkFrame({ children, gutter = "standard", clip = true }: Props) {
  ...
  return (
    ...
    >
      <View
        collapsable={false}
        className="rounded-sm border border-outline bg-surface-variant"
