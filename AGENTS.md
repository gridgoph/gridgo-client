...
- `lib/appUpdate.ts` — the in-app update prompt, shared verbatim with gridgo-rider and gridgo-supplier (only `APP_UPDATE_SOURCE` names this app). It reads GitHub's `releases/latest` unauthenticated, takes the `versionCode` from the tag's run number, and offers `https://gridgo.talasora.com/downloads/gridgo-client.apk`; "Update completed" is said on the next launch because Android's installer owns the install. Only a CI-stamped build (`1.0.<n>` / `versionCode` n) is compared; Expo Go, dev and local builds are skipped unless `EXPO_PUBLIC_UPDATE_CHECK_FORCE_VERSION_CODE=<n>` pretends to be build n. Failure is silence, never a message.
