 ...
 * Delivery tracking is omitted when no client location feed exists on the API.
 */
    ...
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: colors.canvas }} edges={["top"]}>
        <View className="gg-page pt-6">
          <StatusChip tone="error" label="Error" icon="circle-x" />
          <Text className="mt-3 text-body text-error">{error}</Text>
        </View>
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.canvas }} edges={["bottom"]}>
      <ScrollView className="gg-screen">
        ...
        <View className="gap-2">
          ...
          <Text className="text-caption text-text-muted">{order.id}</Text>
        </View>
        ...
        {isTrackingState(order.state) && (
          <View className="gg-card gap-2">
            <Text className="text-h3 text-text-primary">Delivery tracking</Text>
            <StatusChip tone="info" label="Watch only — client does not control dispatch" icon="clock" />
            <Text className="text-body text-text-secondary">
              This order is in {meta.label.toLowerCase()}. Live rider location is not
              available on the demo API for clients (no GET location feed). When that
              endpoint lands, this card will show rider, ETA, and last-updated / stale state.
            </Text>
          </View>
        )}
