...
export function assertLiveGeneration(expected: number): void {
  if (expected !== generation)
    throw new Error("The account changed. Open this screen again.");
}
...
export function readInvalidation(data: string): Invalidation | null {
  try {
    ...
    if (!value || !LIVE_RESOURCES.includes(value.resource as LiveResource))
      return null;
    if (value.id !== undefined && typeof value.id !== "string") return null;
    return {
      resource: value.resource as LiveResource,
