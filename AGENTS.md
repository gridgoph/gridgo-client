...
- **Money** — PHP minor units only. The client sees **printing, delivery, the service fee and total**. Catalog, match, and listing unit prices are **GRIDGO amounts** (shop + live `serviceFeeRateBps` via `lib/gridgoPrice.ts` / `clientFromPriceMinor`). Shop fields stay on the payload for ops. The fee rate comes from `GET /settings` (or the order's snapshotted `serviceFeeRateBps`) — never a hardcoded 3%. Tap the fee for the operations/customer-care explainer. The shop's own settlement is still withheld. The acknowledgement receipt is `GET /orders/:id/invoice`, opened from checkout success and order detail (`app/order/receipt.tsx`).
